...
function initMap(): void {
  ...
  autocomplete.addListener("place_changed", () => {
    ...
    var status = '\n';
    var url = 'https://tracker-340414-default-rtdb.firebaseio.com/places/'+place.place_id+'/status.json';
    ...
    if (err != null) {
        console.error(err);
            ...
            for (let i in data) {
              var available = parseInt(data[i][2]) - parseInt(data[i][1]);
              console.log(available + ' available ' + data[i][0] + '-person table(s)\n');
              status += available + ' available ' + data[i][0] + '-person table(s)\n';
            }
        }
    });
    console.log(status);
    (
      infowindowContent.children.namedItem("place-id") as HTMLElement
    ).textContent = place.place_id as string + status;
